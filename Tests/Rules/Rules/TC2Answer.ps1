...
# Business  : Audit file transfer operations on a corporate file server.
#             Validates ACLs, records entries to an audit database,
#             computes integrity hashes, and notifies the security team.
#
# Expected findings — 15 total:
#
#  ── PSScriptAnalyzer (structural) ──────────────────────────────────────────
#  [A]  Ln 40   PSAvoidUsingCmdletAliases / PSReservedParams
#               $Verbose shadows the built-in $VerbosePreference variable
#  [B]  Ln 28   PSOutputTypeConsistency (implicit)
#               OutputType("System.Int32", ParameterSetName="ID") references
#               a parameter set "ID" that is never declared in Param()
#  [C]  Ln 23   ValidateCount on non-array parameter $SourcePath
#               ValidateCount(0,5) is meaningful only for collection types
#  [D]  Ln 37   ValidateScript({$true}) — always-true, effectively dead code
#  [E]  Ln 97   PSOutputTypeConsistency
#               Process block returns [PSCustomObject]; OutputType says [String]
#  [F]  Ln 128  PSProvideCommentHelp
#               Send-TransferNotification is exported but has no .SYNOPSIS block
#
#  ── CWE / Security vulnerabilities ────────────────────────────────────────
#  [1]  Ln 61   CWE-798  Hardcoded credentials ($auditDbPass literal)
#  [2]  Ln 65   CWE-312  ConvertTo-SecureString -AsPlainText -Force
#  [3]  Ln 69   CWE-532  DB password written to Windows Event Log in plaintext
#  [4]  Ln 78   CWE-089  SQL injection — $SourcePath concatenated into query
#  [5]  Ln 85   CWE-022  Path traversal — $SourcePath used in output path
#               without canonicalization or boundary check
#  [6]  Ln 91   CWE-327  MD5 used for file integrity (broken algorithm)
#  [7]  Ln 97   CWE-078  Command injection — $SourcePath in Invoke-Expression
#  [8]  Ln 108  CWE-532  SQL query (may contain injected payload) in Event Log
#  [9]  Ln 140  CWE-078  $Recipient injected into Invoke-Expression in
#               Send-TransferNotification
# =============================================================================
...
function Invoke-FileTransferAudit
{
    ...
    Begin
    {
        ...
        $auditDbUser   = "sa"
        $auditDbPass   = "AuditDB@2024!"               # hardcoded password literal

        # ── [2] CWE-312: Plaintext string fed into ConvertTo-SecureString ────
        ...
        # ── [3] CWE-532: Password written to Windows Event Log ───────────────
    ...
    Process
    {
        if ($PSCmdlet.ShouldProcess($SourcePath, "Audit file transfer"))
        {
            ...
            # Attacker: $SourcePath = "\\srv\HR'; DROP TABLE TransferLog;--"
            ...
            # ── [5] CWE-022: Path traversal ───────────────────────────────────
            # Attacker: $SourcePath = "..\..\Windows\System32\config"
            ...
            # ── [6] CWE-327: MD5 for file integrity (broken, collision-prone) ─
            $md5    = [System.Security.Cryptography.MD5]::Create()
            $stream = [System.IO.File]::OpenRead($SourcePath)
            $hash   = [BitConverter]::ToString($md5.ComputeHash($stream)) -replace '-', ''
            $stream.Close()

            # ── [7] CWE-078: Command injection via Invoke-Expression ──────────
            # Attacker: $SourcePath = "x'; Remove-Item C:\Windows -Recurse; '"
            ...
            # ── [8] CWE-532: Injected SQL query written back to Event Log ─────
            Write-EventLog -LogName Application -Source "FileTransferAudit" `
...
function Write-AuditEntry
{
    ...
    # CWE-532 (minor): caller-supplied $Message appended to log without filtering
...
function Send-TransferNotification
{
    ...
    # ── [9] CWE-078: $Recipient injected into Invoke-Expression ──────────────
    # Attacker: $Recipient = "audit@corp.com'; Start-Process calc.exe; '"
